...
function GetHtmlEmailByID(linkschecked) {
  ...
  for (let i = 0; i < linkschecked.length; i++) {
    const diccionarioLink = linkschecked[i].split('|');
...
function getEmailLinks(id, rawHTML, linkstoreplace, toReplace, emailName) {
  ...
  for (let i = 0; i < links.length; i++) {
    const LinkHtml = links[i].outerHTML;
    let HtmlLinkText = LinkHtml.split('>')[1].split('<')[0].trim();
...
function replaceUrlTOkens(token) {
  $('#htmlemailsLink')[0].href = `/htmlemails/home?rt=${
    token
  }&eid=${
    $('#eid').val()
  }&tssd=${
    $('#tssd').val()}`;
  $('#DashboardLink')[0].href = `/Dashboard/home?rt=${
    token
  }&eid=${
    $('#eid').val()
  }&tssd=${
...
function replaceLinks(rawHTML, object, OneLink) {
  let htmlEmail = rawHTML;
  for (let i = 0; i < object.Links.length; i++) {
    const oldString = object.Links[i].htmlLink;
    const oldStringLength = oldString.length;
    const htmlBeforeLink = htmlEmail.substring(0, htmlEmail.indexOf(oldString));
    console.log(htmlBeforeLink);
    const htmlafterLink = htmlEmail.substring(
      htmlBeforeLink.length + oldStringLength,
      htmlEmail.length,
    );
    const newString = oldString.replace(object.Links[i].href, OneLink);
...
function buildPaginator(allEmails) {
  ...
  let totalPages = Math.ceil(allEmails.length / 15);
  if (totalPages == 0) {
    totalPages++;
  }
...
function listLinks(data) {
  ...
  if (array !== undefined && array.length > 0) {
    array.sort((a, b) => (new Date(a.Modified) < new Date(b.Modified)
      ? 1
  ...
  for (let index = 0; index < array.length; index++) {
    ...
    const option = `<option id="link${index}" class="slds-listbox__item" value="${element.LinkID}|${element.FullURL}">${element.LinkName}</option>`;
...
function buildEmailSlot(emailforslot, length) {
  if (emailforslot.length == length) {
    for (let j = 0; j < emailforslot.length; j++) {
      ...
      const emailCampaign = emailforslot[j].emailBody.data.campaigns;
      if (
        emailCampaign != undefined
        && Object.keys(emailCampaign).length != 0
      ) {
        if (emailCampaign.campaigns[0] != undefined) {
          const campaign = getCampaignById(emailCampaign.campaigns[0].campaignId);
...
function getAllEmailsWithOneLinks(emailUpdated) {
  ...
  $.ajax({
    ...
    success: (data) => {
      $('#rt').val(data.refresh_token);
      const emailswithonelink = data.body;
