  ...
  if (json.length > 0) {
    ...
    if (alreadyExist === false) {
      ...
      if (isAttributionLink.Name !== undefined) {
        JSONParameter.AttributtionLinks.push({
          index: rules.length,
          name: name,
          value: value,
          canDelete: true,
          selectId: `select${rules.length}`,
          inputValueId: name,
          isCustom: false,
          customValue: null,
        });
...
function createHtmlForRule(
  ...
  newRule += "</div>";
  newRule +=
    '<div class="Select rules-select-param-name   Select--single is-searchable has-value">';
  ...
  newRule +=
    '<div style="position: absolute; visibility: hidden; height: 0px; width: 0px; overflow: scroll; white-space: pre; font-size: 14px; font-family: museo_sans300; font-weight: 400; font-style: normal; letter-spacing: normal;">';
  newRule += "</div>";
  newRule += "</div>";
  ...
  newRule += "</div>";
  newRule +=
    '<div class="af-core form-control-wrapper af-core rules-input-param-value  input-item    vertical">';
  newRule += "<div>";
  newRule += "<div></div>";

  if (value) {
    newRule +=
      '<div class="input-container has-validation form-group has-feedback">';
  ...
  newRule +=
    '<option value="" class="options-personalizationstring">Select an option</option>';
  newRule +=
    '<option value="emailaddr" class="options-personalizationstring">Email Address</option>';
  newRule +=
    '<option value="fullname" class="options-personalizationstring">Full Name</option>';
  newRule +=
    '<option value="firstname" class="options-personalizationstring">First Name</option>';
  newRule +=
    '<option value="lastname" class="options-personalizationstring">Last Name</option>';
  newRule +=
    '<option value="_subscriberkey" class="options-personalizationstring">Subscriber Key</option>';
  newRule +=
    '<option value="mobile_number" class="options-personalizationstring">Phone Number</option>';
  newRule += "</select>";
...
function buildSelectOptions(element, selectOptions) {
  ...
  options +=
    '<option class="option-select" value="select" selected>Select Parameter</option>';
  for (let j = 0; j < selectOptions.length; j++) {
    const option = selectOptions[j];
    if (
      element.name.startsWith("custom") &&
      element.name !== "Custom Parameter"
    ) {
...
function removeAttrParamsFromCustomParams(customparams) {
  ...
  if (json.length > 0) {
    ...
    for (let index = 1; index < params.length; index++) {
      ...
      if (isCustom === true) {
        JSONParameter.CustomParameters.push({
          name: queryParam[0],
          value: queryParam[1],
        });
        const newParam = `${queryParam[0]}=${queryParam[1]}`;
        if (customInputValue.indexOf(`${queryParam[0]}=`) > 0) {
          const oldParam = customInputValue.substring(
            customInputValue.indexOf(`${queryParam[0]}=`),
            customInputValue.length
          );
          customInputValue = customInputValue.replace(
            oldParam.split("&")[0],
            newParam
          );
        } else {
...
function fillUi(data) {
  ...
  for (let index = 0; index < arrayParams.length; index++) {
    ...
    if (
      element[0] !== "pid" &&
      element[0] !== "af_channel" &&
      element[0] !== "c" &&
      element[0] !== "is_retargeting" &&
      element[0] !== "Select parameter"
    ) {
...
function getLinkData(postData) {
  $.ajax({
    ...
    error(jqXHR, error, errorThrown) {
      ...
      setInterval(e => {
        e.preventDefault();
        window.location.href = `/dashboard/home/?rt=${$("#rt").val()}eid=${$(
          "#eid"
        ).val()}&tssd=${$("#tssd").val()}`;
      }, 3000);
    },
...
$(document).ready(() => {
  ...
  $("#cancel").on("click", e => {
    e.preventDefault();
    if ($("#rt").val() === undefined || $("#rt").val() === "") {
      window.location.href = `/dashboard/home/?rt=${
        urlParams.refresh_token
      }&eid=${urlParams.enterpriseId}&tssd=${$("#tssd").val()}`;
    } else {
      window.location.href = `/dashboard/home/?rt=${$("#rt").val()}&eid=${$(
        "#eid"
      ).val()}&tssd=${$("#tssd").val()}`;
  ...
  $("#btn-updateLink").on("click", e => {
    ...
    if (customParameters.length > 0) {
      customParameters =
        customParameters.startsWith("&") === true
          ? customParameters
          : `&${customParameters}`;
    }
