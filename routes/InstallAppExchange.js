...
const retrieveFolder = (enterpriceId, name, client) =>
...
const GetAppsFlyersFolderID = (enterpriseId, client) =>
 new Promise((resolve, reject) => {
  if (client === undefined) {
    ...
    retrieveFolder(enterpriseId, "Shared Data Extensions", client)
     .then((result) => {
      if (result.Results !== undefined) {
       ...
       client.Create(
        CreateRequestFolder(process.env.Folder, parentId),
        (err, res) => {
         if (err) {
          return reject(err);
         }
